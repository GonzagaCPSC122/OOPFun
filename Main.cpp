...
int main() {
  ...
  hp1.display(); // hp1 is an "invoking" object... so in display() it is hp1's
                 // title, author, numPages that are accessed
  ...
  Book hp3 =
      hp2; // copy of hp2... invokes the copy constructor if one exists, if one
           // doesn't exist then performs memberwise copy assignment
  ...
  // so we get a double free detected when we destruct hp2 and call delete
  // numPages SOLUTION? define a copy constructor where we allocate new member
  // for numPages in hp3
